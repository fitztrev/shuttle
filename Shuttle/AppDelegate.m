...
- (BOOL)isJSONFile:(NSURL *)fileURL {
    ...
    return [filePath hasSuffix:@".json"];
}
...
- (IBAction)showImportPanel:(id)sender {
    ...
    if (tvarNSInteger == NSOKButton) {
        ...
        // Check if the file isn't JSON
        if (![self isJSONFile:selectedFileUrl]) {
            NSAlert *alert = [NSAlert alertWithMessageText:@"JSON Error"
                                             defaultButton:@"Yes"
                                           alternateButton:@"No"
                                               otherButton:nil
                                 informativeTextWithFormat:@"There may be issues parsing this file. Import anyway?"];
